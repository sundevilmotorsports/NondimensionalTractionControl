...
#undef AI_TIRE_SLIP_NETWORK_N_BATCHES
#define AI_TIRE_SLIP_NETWORK_N_BATCHES         (1)

static ai_ptr g_tire_slip_network_activations_map[1] = AI_C_ARRAY_INIT;
static ai_ptr g_tire_slip_network_weights_map[1] = AI_C_ARRAY_INIT;
...
/******************************************************************************/
AI_DECLARE_STATIC
ai_bool tire_slip_network_configure_activations(
  ai_network* net_ctx, const ai_network_params* params)
{
  ...
  if (ai_platform_get_activations_map(g_tire_slip_network_activations_map, 1, params)) {
    ...
    nl_1_output_array.data_start = AI_PTR(g_tire_slip_network_activations_map[0] + 48);
    gemm_2_output_array.data = AI_PTR(g_tire_slip_network_activations_map[0] + 0);
    gemm_2_output_array.data_start = AI_PTR(g_tire_slip_network_activations_map[0] + 0);
    nl_3_output_array.data = AI_PTR(g_tire_slip_network_activations_map[0] + 48);
    nl_3_output_array.data_start = AI_PTR(g_tire_slip_network_activations_map[0] + 48);
    gemm_4_output_array.data = AI_PTR(g_tire_slip_network_activations_map[0] + 0);
    gemm_4_output_array.data_start = AI_PTR(g_tire_slip_network_activations_map[0] + 0);
    return true;
  }
...
/******************************************************************************/
AI_DECLARE_STATIC
ai_bool tire_slip_network_configure_weights(
  ai_network* net_ctx, const ai_network_params* params)
{
  AI_ASSERT(net_ctx)

  if (ai_platform_get_weights_map(g_tire_slip_network_weights_map, 1, params)) {
...
AI_DEPRECATED
AI_API_ENTRY
ai_bool ai_tire_slip_network_get_info(
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_TIRE_SLIP_NETWORK_MODEL_NAME,
      .model_signature   = AI_TIRE_SLIP_NETWORK_MODEL_SIGNATURE,
...
AI_API_ENTRY
ai_bool ai_tire_slip_network_get_report(
  ai_handle network, ai_network_report* report)
{
  ai_network* net_ctx = AI_NETWORK_ACQUIRE_CTX(network);
...
AI_API_ENTRY
ai_error ai_tire_slip_network_get_error(ai_handle network)
{
...
AI_API_ENTRY
ai_error ai_tire_slip_network_create(
...
AI_API_ENTRY
ai_error ai_tire_slip_network_create_and_init(
  ai_handle* network, const ai_handle activations[], const ai_handle weights[])
{
  ai_error err;
  ai_network_params params;

  err = ai_tire_slip_network_create(network, AI_TIRE_SLIP_NETWORK_DATA_CONFIG);
  if (err.type != AI_ERROR_NONE) {
    return err;
  }
  
  if (ai_tire_slip_network_data_params_get(&params) != true) {
    err = ai_tire_slip_network_get_error(*network);
    return err;
  }
#if defined(AI_TIRE_SLIP_NETWORK_DATA_ACTIVATIONS_COUNT)
  /* set the addresses of the activations buffers */
  for (ai_u16 idx=0; activations && idx<params.map_activations.size; idx++) {
    AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_activations, idx, activations[idx]);
  }
#endif
#if defined(AI_TIRE_SLIP_NETWORK_DATA_WEIGHTS_COUNT)
  /* set the addresses of the weight buffers */
  for (ai_u16 idx=0; weights && idx<params.map_weights.size; idx++) {
    AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_weights, idx, weights[idx]);
  }
#endif
  if (ai_tire_slip_network_init(*network, &params) != true) {
    err = ai_tire_slip_network_get_error(*network);
  }
  return err;
}
...
AI_API_ENTRY
ai_buffer* ai_tire_slip_network_inputs_get(ai_handle network, ai_u16 *n_buffer)
{
...
AI_API_ENTRY
ai_buffer* ai_tire_slip_network_outputs_get(ai_handle network, ai_u16 *n_buffer)
{
  if (network == AI_HANDLE_NULL) {
    network = (ai_handle)&AI_NET_OBJ_INSTANCE;
    AI_NETWORK_OBJ(network)->magic = AI_MAGIC_CONTEXT_TOKEN;
...
AI_API_ENTRY
ai_handle ai_tire_slip_network_destroy(ai_handle network)
{
...
AI_API_ENTRY
ai_bool ai_tire_slip_network_init(
  ai_handle network, const ai_network_params* params)
{
  ...
  ok &= tire_slip_network_configure_weights(net_ctx, params);
  ok &= tire_slip_network_configure_activations(net_ctx, params);
...
AI_API_ENTRY
ai_i32 ai_tire_slip_network_run(
...
AI_API_ENTRY
ai_i32 ai_tire_slip_network_forward(ai_handle network, const ai_buffer* input)
{
